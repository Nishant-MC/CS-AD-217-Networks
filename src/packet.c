...
int verifyPacket(Packet *pkt)
{
    ...
    // Magic number checking - CMU default values used
...
Packet *newPacketDefault()
{
    ...
    *((uint16_t *)ptr) = 15441; // CMU's Network's course = 15-441, same as the magic number
    *(ptr + 2) = 1; // Version number
...
Packet *newPacketSingleDATA(int seqNo, int seqChunk, size_t size)
{
    ...
    if(retSize != size) {
        printf("Encountered IO Error while reading a chunk!\n");
        freePacket(pkt);
        return NULL;
    } else {
        return pkt;
...
uint8_t *getPacketHash(Packet *pkt, int i)
{
    ...
    if(type == 0 || type == 1){ // 0 = WHOHAS and 1 = IHAVE
      return pkt->payload + 20 + SHA1_HASH_SIZE * i;
    } else if (type == 2){ // 2 = GET
