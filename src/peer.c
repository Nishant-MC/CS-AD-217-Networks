...
void fillChunkList(chunkList *list, enum chunkType type, char *filename)
{
  ...
  case MASTER:
    ...
    if(strncmp(lineBuf, "File: ", 6) != 0) {
      fprintf(stderr,"Error encountered while parsing masterchunks!\n");
      exit(1);
    } else {
      FILE *masterFile;
	...
	*newline = '\0';
	...
	fprintf(stderr,"Error encountered while opening master data file: <%s>\n", linePtr);
	exit(1);
      }
  ...
  case HAS:
	...
	fprintf(stderr,"Error encountered while parsing hashes!\n");
	exit(1);
      }
...
void process_get(char *chunkfile, char *outputfile)
{
  ...
  if((fopen(chunkfile, "r") == NULL)){
    fprintf(stderr, "Opening %s failed. (Mis-spelled?)\n", chunkfile);
    return;
  }
...
void flushUpload(int sock)
{
  ...
  for(i = 0; i < peerInfo.numPeer; i++) {
    ...
    while(pkt != NULL &&
      ...
      if(retVal == -1) { // If spiffy_sendto doesn't fucking work (AND IT DOESN'T!)
